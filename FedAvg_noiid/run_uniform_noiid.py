...
def main():
    # Uniform分布专用种子
    SEED = 12345  # 与其他分布不同的种子
    set_seed(SEED)
    
    # 设置日志
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = f"results/logs/uniform_noiid_{timestamp}.log"
    logger = setup_logging(log_file)
    
    logger.info("=" * 80)
    logger.info("开始 UNIFORM 分布实验 (Non-IID版本)")
    ...
    logger.info("🎯 Non-IID实验：UNIFORM")
    logger.info(f"📊 分布类型: uniform")
    logger.info(f"⚙️  随机种子: {SEED}")
    ...
    logger.info(f"   选择器类型: Uniform")
    logger.info(f"   选择器配置: 均匀随机选择")
    ...
    # 保存模型
    model_path = f"results/models/uniform_noiid_model.pt"
    torch.save(trainer.model.state_dict(), model_path)
    logger.info(f"   模型已保存: {model_path}")
    
    # 保存训练历史
    results_path = f"results/uniform_noiid_results.pt"
    results = {
    ...
    final_accuracy = trainer.test_accuracies[-1] if trainer.test_accuracies else 0
    logger.info("=" * 80)
    logger.info("🎉 UNIFORM分布实验完成 (Non-IID版本)")
