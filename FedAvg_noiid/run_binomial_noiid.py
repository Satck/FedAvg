...
def main():
    # Binomial分布专用种子
    SEED = 23456  # 与其他分布不同的种子
    set_seed(SEED)
    
    # 设置日志
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = f"results/logs/binomial_noiid_{timestamp}.log"
    logger = setup_logging(log_file)
    
    logger.info("=" * 80)
    logger.info("开始 BINOMIAL 分布实验 (Non-IID版本)")
    ...
    binomial_params = {
        'alpha': 3,
        'beta': 7,
        'binomial_mode': 'heterogeneous'
    }
    
    logger.info("🎯 Non-IID实验：BINOMIAL")
    logger.info(f"📊 分布类型: binomial")
    logger.info(f"⚙️  分布参数: {binomial_params}")
    ...
    selector = BinomialSelector(
        num_clients=config['num_clients'],
        clients_per_round=config['clients_per_round'],
        **binomial_params
    )
    logger.info(f"   选择器类型: Binomial")
    logger.info(f"   选择器配置: {binomial_params}")
    ...
    # 保存模型
    model_path = f"results/models/binomial_noiid_model.pt"
    torch.save(trainer.model.state_dict(), model_path)
    logger.info(f"   模型已保存: {model_path}")
    
    # 保存训练历史
    results_path = f"results/binomial_noiid_results.pt"
    results = {
        ...
        'distribution_params': binomial_params,
        'seed': SEED,
        'timestamp': timestamp,
        'noiid': True,
    ...
    final_accuracy = trainer.test_accuracies[-1] if trainer.test_accuracies else 0
    logger.info("=" * 80)
    logger.info("🎉 BINOMIAL分布实验完成 (Non-IID版本)")
