...
def main():
    # Poisson分布专用种子
    SEED = 34567  # 与其他分布不同的种子
    set_seed(SEED)
    
    # 设置日志
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = f"results/logs/poisson_noiid_{timestamp}.log"
    logger = setup_logging(log_file)
    
    logger.info("=" * 80)
    logger.info("开始 POISSON 分布实验 (Non-IID版本)")
    ...
    poisson_params = {
        'lambda': 2.5,
        'poisson_mode': 'time_varying'
    }
    
    logger.info("🎯 Non-IID实验：POISSON")
    logger.info(f"📊 分布类型: poisson")
    logger.info(f"⚙️  分布参数: {poisson_params}")
    ...
    selector = PoissonSelector(
        num_clients=config['num_clients'],
        clients_per_round=config['clients_per_round'],
        **poisson_params
    )
    logger.info(f"   选择器类型: Poisson")
    logger.info(f"   选择器配置: {poisson_params}")
    ...
    # 保存模型
    model_path = f"results/models/poisson_noiid_model.pt"
    torch.save(trainer.model.state_dict(), model_path)
    logger.info(f"   模型已保存: {model_path}")
    
    # 保存训练历史
    results_path = f"results/poisson_noiid_results.pt"
    results = {
        ...
        'distribution_params': poisson_params,
        'seed': SEED,
        'timestamp': timestamp,
        'noiid': True,
    ...
    final_accuracy = trainer.test_accuracies[-1] if trainer.test_accuracies else 0
    logger.info("=" * 80)
    logger.info("🎉 POISSON分布实验完成 (Non-IID版本)")
