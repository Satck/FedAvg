# experiments/run_noniid_experiments.py
...
def run_noniid_baseline():
    ...
    print("="*80)
    print("实验：MNIST Non-IID - FedAvg Baseline (均匀分布)")
    print("="*80)
    ...
    # 创建Non-IID划分
    print("2. 创建Non-IID数据划分...")
    client_indices = create_noniid_partition(
        train_dataset, 
    ...
    torch.save(results, os.path.join(results_dir, 'noniid_baseline_results.pt'))
    torch.save(model.state_dict(), os.path.join(models_dir, 'noniid_baseline_model.pt'))
    
    print("\n实验完成！")
    print(f"结果已保存至: {os.path.join(results_dir, 'noniid_baseline_results.pt')}")
    
    return results


if __name__ == '__main__':
    results = run_noniid_baseline()
